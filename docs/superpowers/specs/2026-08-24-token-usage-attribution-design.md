...
# 레이스 패널 재설계 (2026-08-24)

첫 구현은 막대그래프였고 레이스가 아니었다. 스크린샷을 받고 결함 5개를 특정했다.

| # | 결함 | 원인 | 조치 |
|---|---|---|---|
| 1 | 금액이 안 보임 | 채움도 `--c1`, 값 텍스트도 `--c1` | 값을 채움 밖으로, text 토큰 적용 |
| 2 | 레이스 성립 안 함 | `peak` = 레인 최대 총합 → 레인 1개면 항상 100% | 솔로는 지난달 자신을 상대로 |
| 3 | 미래까지 달림 | 클라이언트가 주 끝까지 애니메이션 | 서버가 `today_index` 내려주고 거기서 정지 |
| 4 | 단위가 금액 | 토큰이 주 단위여야 함 | 억·만 표기 + 금액 괄호. `raceLanes` 가 둘 다 낸다 |
| 5 | 진행이 안 읽힘 | 주 단위 7프레임 | **월 단위 30여 프레임** |

## 형태 선택 (dataviz 절차)

anti-patterns 의 "a one-bar bar chart → 스탯 타일" 과 "Text wears text tokens, never
the series color" 를 둘 다 어긴 상태였다.

- **솔로**: 지난달을 완주한 페이스 라인으로 깔고 이번 달만 오늘까지 달린다. 같이 잘라
  내면 상대가 사라진다. 누적 곡선 + 2개 레인(이번 달/지난달).
- **전체**: **emphasis** (1 hue + gray) — 내 레인 accent, 동료는 de-emphasis 회색.
  categorical 8색 팔레트가 필요 없다. 내 대시보드이므로 내 레인이 주인공인 것이 맞다.

## 색 (검증기 결과)

호스트 앱 토큰을 파라미터로 넣고 `validate_palette.js` 를 돌렸다.

| 후보 | 대비(흰 배경) | CVD ΔE | 판정 |
|---|---:|---:|---|
| `--accent` #03c75a | 2.25:1 | 12.9 | 대비 3:1 미달 |
| `--accent-strong` #02a04a | ≥3:1 | 10.5 | **채택** |

de-emphasis 회색 `#8b95a1` 의 chroma FAIL 은 emphasis 형태의 의도된 값이고 검증기
scope("categorical palettes only") 밖이다.

## 눈으로 확인한 것 (절차 7번)

dev 서버에 실제 컴포넌트를 띄우고 중앙 저장소 실데이터로 렌더해 확인했다. 지난번에
이 단계를 건너뛰어 안 보이는 화면을 보냈다. 이 과정에서만 잡힌 결함:

- 첫 프레임의 단일 점 경로는 SVG 가 아무것도 그리지 않는다
- `preserveAspectRatio="none"` 아래에서 `<circle>` 이 타원으로 늘어난다
- 선두 마커를 CSS 로 옮기면 transition 때문에 선보다 늦게 따라와 오히려 느려 보인다
  → 마커를 두지 않는 것으로 결론
...
| 머신 | 일자별 행 | 토큰 | 최근 |
|---|---:|---:|---|
| mac-studio | 242 | 72.7억 | 2026-08-24 |
| macbook | 35 | 17.6억 | 2026-07-28 |
