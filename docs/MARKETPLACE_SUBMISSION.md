# Anthropic Marketplace Submission — Vibe-Harness

제출 URL: https://platform.claude.com/plugins/submit
레포: https://github.com/hoarchi/vibe-harness

---

## Description

Vibe-Harness is a dev progress kanban board that runs entirely inside Claude Code. When you code with Claude, every task is automatically tracked — started, completed, and reported — on a local kanban board with a browser-based UI.
...
You're building a side project and want to see what you actually accomplished each day. Instead of maintaining a PROGRESS.md by hand, Vibe-Harness captures every task Claude works on. At the end of the day, open `localhost:4242/kanban` and see your done column with code change stats (+142 / -38 lines), elapsed time, and work reports for each task.

### 2. Vibe Coding Sessions — Staying Organized

During a long vibe coding session, it's easy to lose track of what's done and what's left. Vibe-Harness keeps a live board that auto-refreshes every 5 seconds. You can see at a glance: 2 tasks in progress, 5 in backlog, 12 done today. The list view gives you a spreadsheet-style overview of everything, with inline editing.

### 3. Multi-Project Management — One Dashboard

You're working on 3 projects this week. Each has its own SQLite DB, but one Vibe-Harness server serves them all. Tab between projects in the header, see per-project stats, and never mix up which tasks belong where.
...
Tired of manually updating PROGRESS.md? Run `/vibe-harness sync` to import existing records, then let the kanban handle all future tracking. Claude automatically writes work details on task completion — no more "update the progress doc" at the end of every session.
